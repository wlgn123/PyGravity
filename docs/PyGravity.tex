...
To use create a Physics Class object then particles can be add using the Physics Class method. 
...
\end{equation}
where $P_1$ and $P_2$ are the position vectors for both respective particles in question. Then the magnitude of $\vec{R}$ is noted as $\left| \vec{R} \right| $. Thus the unit vector pointing in the direction of $\vec{R}$ is $\frac{\vec{R}}{\left| \vec{R} \right|}$. Now we can rewrite the equation \ref{grav} as 
\begin{equation}
F = \frac{G m_1 m_2}{\left| \vec{R} \right| ^2}  \frac{\vec{R}}{\left| \vec{R} \right|}
\end{equation}
\printbibliography
\end{document}
